...
The code multiplies these vectors by sqrt{[d_model]} to ensure that the variance of the embeddings matches the variance of the positional encodings.
...
- Scaled Dot-Product: The core math is Softmax((QK^T)/(sqrt{[d_k]})). QK^T calculates a score for how much every word should care about every other word. Dividing by sqrt{[d_k]} prevents the scores from getting too large, which would "kill" the gradients during training.
